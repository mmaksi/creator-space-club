...
describe('User Sign-up', () => {
    ...
    it('Returns 400 with an invalid password', async () => {
        ...
        expect(response.status).toEqual(400);
        expect(response.body.errors[0].message).toBe('Password must be between 4 and 20 characters');
    });
    ...
    it('Returns 400 with missing required fields', async () => {
        ...
        await request(app).post('/api/users/signup').send({ password: 'long_password' }).expect(400);
    });
...
describe('Current User', () => {
    it('Responds with current details of the current user', async () => {
        ...
        const response = await request(app).get('/api/users/current-user').send({}).set('Cookie', cookie).expect(200);

        expect(response.body.currentUser.id).toBeDefined();
    });

    it('Responds with null if the user is not authentiacted', async () => {
        const response = await request(app).get('/api/users/current-user').send({}).expect(401);
